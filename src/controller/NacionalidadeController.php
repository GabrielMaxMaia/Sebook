...
class NacionalidadeController
{
    ...
    {
        $this->nacionalidadeDAO = new NacionalidadeDAO($sql);
    ...
    public function setNacionalidadeDAO($nacionalidadeDAO)
    {
    ...
    public function recuperarDadosFormulario()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $this->nacionalidadeDAO->setIdNacionalidade($_POST['txtId']);
            $this->nacionalidadeDAO->setNomeNacionalidade($_POST['txtNome']);
        }
    }
    ...
    public function gravarAlterar()
    {
        ...
        if ($this->acaoPOST == 1 && $this->evitarReenvio()) {
            $this->nacionalidadeDAO->adicionarNacionalidade();
        } else if ($this->acaoPOST == 2) {
            $this->nacionalidadeDAO->alterarNacionalidade();
        }
    }
    ...
    {
        if ($this->acaoGET == 3) {
            $this->nacionalidadeDAO->setIdNacionalidade($_GET['id']);
            $this->nacionalidadeDAO->excluirNacionalidade();
        }
    }
    ...
    public function listarNacionalidadeId()
    {
        if ($this->acaoGET == 2) {
            $this->nacionalidadeDAO->setIdNacionalidade($_GET['id']);
            $Nacionalidade = $this->nacionalidadeDAO->listarNacionalidadeId();
            $this->nacionalidadeDAO->setNomeNacionalidade($Nacionalidade['nomeNacionalidade']);
        }
    }

    public function listarNacionalidades()
    {
        $result = $this->nacionalidadeDAO->listarNacionalidades($this->regIni, $this->itemPagina);
    ...
    public function exibirNotificador($urlDoNotificador)
    {
       // Passando a quantidade de paginas como parmetro
        $qtdePaginas = ceil($this->nacionalidadeDAO->totalContar() / $this->itemPagina);

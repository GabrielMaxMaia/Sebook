...
class PostagemController
{
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $this->eventoDAO->setIdPostagem($_POST['txtId']);
            $this->eventoDAO->setIdUsuario($_SESSION['userLogado']['idUsuario']);
            $this->eventoDAO->setTituloPostagem($_POST['txtTitulo']);
            $this->eventoDAO->setTxtPostagem($_POST['txtPostagem']);
            $this->eventoDAO->setDatahoraPostagem($_POST['dataPost']);
            $this->eventoDAO->setUrlFotoPostagem($_POST['txtImg']);
        }
    }
    ...
    public function gravarAlterar()
    {
        ...
        if ($this->acaoPOST == 1 && $this->evitarReenvio()) {
            $this->eventoDAO->adicionarPostagem();
        } else if ($this->acaoPOST == 2) {
            $this->eventoDAO->alterarPostagem();
        }
    }
    ...
    {
        if ($this->acaoGET == 3) {
            $this->eventoDAO->setIdPostagem($_GET['id']);
            $this->eventoDAO->excluirPostagem();
        }
    }
    ...
    public function listarPostagemId()
    {
        if ($this->acaoGET == 2) {
            $this->eventoDAO->setIdPostagem($_GET['id']);
            $postagem = $this->eventoDAO->listarPostagemId();
            $this->eventoDAO->setTituloPostagem($postagem['tituloPostagem']);
            $this->eventoDAO->setTxtPostagem($postagem['txtPostagem']);
            $this->eventoDAO->setUrlFotoPostagem($postagem['urlFotoPost']);
        }
    }
    ...
    public function listarPostagem()
    {
        $result = $this->eventoDAO->listarPostagem($this->regIni, $this->itemPagina);
        $postagem = "";
